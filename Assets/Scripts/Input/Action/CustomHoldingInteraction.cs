...
namespace Invertex.UnityInputExtensions.Interactions
{
    ...
    public class CustomHoldingInteraction : IInputInteraction
    {
        ...
        private void OnUpdate()
        {
            ...
            if (phase is InputActionPhase.Canceled or InputActionPhase.Disabled || !ctx.action.actionMap.enabled || (!isActuated && phase is InputActionPhase.Performed or InputActionPhase.Started))
            {
                Cancel(ref ctx);
                return;
            }
            ...
            if (phase is InputActionPhase.Performed or InputActionPhase.Started)
            {
        ...
        static void RegisterInteraction()
        {
            ...
            {
                //For some reason if this is called again when it already exists, it permanently removes it from the drop-down options... So have to check first
